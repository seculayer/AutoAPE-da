...
class MPLogger(object):
    ...
    def __init__(self, log_dir, log_name, log_level):
        ...
        # Formatter
        formatter = logging.Formatter("%(asctime)s %(levelname)-5s - %(processName)-15s - "
                                      "%(filename)-22s:%(lineno)-3s - %(message)s")
...
if __name__ == '__main__':
    ...
    workers = []
    for i in range(20):
        wp = Process(target=worker_process, name='worker %d' % (i + 1), args=())
        workers.append(wp)
        wp.start()
    for wp in workers:
        wp.join()
